...
import { getMchatResultString, mchatFormDataToArray, prepareMchatData } from "./Utils";
...
export const Mchat = ({ assistidoId }: Props) => {
  ...
  const questionArray = useMemo(() => mchatFormDataToArray(mchatData), [mchatData]);
  ...
  const handleAnwserChange = (questionIndex: number, anwser: boolean) => {
    setMchatData((actualMchatdata) => {
      actualMchatdata.set(questionIndex, {... actualMchatdata.get(questionIndex)!, resposta: anwser})
  ...
  return (
    <div className="p-6">
      ...
      <MchatQuestions mchatDataArray={questionArray} handleAnwserChange={handleAnwserChange} />

      <div className="mt-6 max-w-[600px] px-2 py-6 bg-blue-900 m-auto flex flex-col justify-center items-center rounded-md">
      {!isCompleted() ?
        <p className="text-lg text-white text-center">{"Responda todas as questões para obter um resultado."}</p> 
        :
        <>
          <p className="text-lg text-white uppercase mb-4">{"resultado"}</p> 
          <p className="text-lg text-white text-center">{getMchatResultString(mchatData)}</p> 
        </>
      }
      </div>
